...
UniValue z_getaddressforaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "\n- If no list of receiver types is given (or the empty list \"[]\"), an Orchard"
            ...
            "  \"receiver_types\": [\"orchard\"],         (json array of string) the receiver types that the UA contains (valid value is \"orchard\")\n"
            "  \"address\"                              (string) The corresponding address\n"
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("z_getaddressforaccount", "0")
            + HelpExampleCli("z_getaddressforaccount", "0 '[]' 1")
            + HelpExampleCli("z_getaddressforaccount", "0 '[\"orchard\"]' 1")
